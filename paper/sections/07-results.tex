...
A separate source ledger screened 25 cited public faults from PostgreSQL, Supabase, ClickHouse, Cube, Looker, dbt/MetricFlow, Metabase, Superset, Hasura, and LangChain. Nineteen were mapped to 12 existing operators and all 19 fixtures were killed; six were dropped because their direction or mechanism was outside the model. Exact Git-object replay over three fixes in a private source repository also verified that the relevant source contract was absent before and present after each fix. Two missing-RLS revisions map to \texttt{app\_deny\_missing\_db\_policy}; an export-audit omission is reproducible but outside v1. The public artifact reports only aggregate counts, not private revisions, paths, source excerpts, or probe patterns. This replays historical source states, not running vulnerable services, and does not show prevention. Section~\ref{sec:external-taxonomy} adds the complementary scope check: measured against an independently authored data-agent vulnerability taxonomy, the v1 registry covers two of eight classes and one partially, and 401 of its own cases describe update-induced drift that an attack taxonomy has no reason to name.
...
Historical revisions & 3/3 & private Git objects & aggregate source-contract reproduction \\
...
A maintainer-operated production study verified that the inspected source and deployed source revisions matched; service, route, revision, and deployment identifiers are withheld because the repository and control plane are private. Thirty-three of 36 read-only HTTP boundary probes passed and none failed. Three authenticated reads were skipped because no isolated synthetic production principal exists; checks that could enqueue work or alter state were excluded. On the same source revision, doctor found 33 tools, three roles, six runtime events, and no partial wiring; six SQL traces plus four database checks passed against disposable synthetic PostgreSQL. No customer row was read and no production state was mutated. Thus the study establishes deployment binding, adapter applicability, and selected live denial behavior---not public reproducibility, authenticated cross-tenant behavior, customer-data safety, or effectiveness on unknown faults.
