...
## PolicyStrata OSS And Clearance

PolicyStrata OSS is the local package: deterministic runner, scanner, SDK contracts, runtime
evaluator, customer-hosted gateway, schemas, tests, and evidence artifacts. It works without a
hosted account and deterministic runs do not require an LLM API key.

Clearance is optional hosted infrastructure for organization state: auth, RBAC, review UI,
waivers, approvals, audit logs, procurement workflows, billing, and trust-center reporting.
...
PolicyStrata produces evidence close to sensitive systems; Clearance records review state and
release decisions. Keep that boundary intact:
