...
test("evaluateRuntimeEvent strips SQL comments without regex backtracking", () => {
  const adversarialBlockComment = `/*${"a/*".repeat(2048)}*/`;
  const allowed = evaluateRuntimeEvent(
    governedRuntimeManifest,
    runtimeEvent({
      payload: {
        sql:
          `${adversarialBlockComment} select * from support_tickets ` +
          `where tenant_id = 'tenant_a' -- ${"x".repeat(2048)}`,
      },
    }),
  );
  ...
  const denied = evaluateRuntimeEvent(
    governedRuntimeManifest,
    runtimeEvent({
      payload: {
        sql:
          `select * from support_tickets /* where tenant_id = 'tenant_a' ${"a/*".repeat(512)} */ ` +
          "where status = 'open'",
      },
    }),
  );
  ...
  assert.equal(denied.allowed, false);
  assert.match(denied.reason, /missing tenant predicate tenant_id/);
});
